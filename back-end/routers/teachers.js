...
=======
	...
	LootReward,
	PointReward
>>>>>>> e7de73d7fb475acd481589c13a8eefde9e6cadf0
} = require("../models");
...
router.patch("/:te_id/student/:st_id/confirmTask/:t_id", async (req, res) => {
  try {
    ...
    // CRITICAL
    // if task has rewards, hit endpoint for distribution
    if (task.rewards) {
      req.session.body = task.rewards;
...
>>>>>>> c910bceb98c0a8d1dc3e03c05adba8f8b0685dd3
...
router.patch("/:te_id/student/:st_id/confirmReward/:t_id", async (req, res) => {
  try {
    ...
    const reward = student.getReward(req.params.t_id);
    if (!reward) {
      throw new Error(`That student doesn't have a reward with that id`);
    }

    if (reward.status !== "Pending") {
      throw new Error(`You can only confirm rewards that are pending rewards`);
    }
...
<<<<<<< HEAD
    ...
    // task completion -> gets reward
    // pointreward auto redeems
    // lootreward is pending until Redeemed
    //
    // reward.
    ...
    // Create log events.
    logEvent(TaskEvent, {
      message: Messages.TEMPLATE_REWARD_CONFIRM_REDEMPTION,
      owner: req.user,
      user: student,
      reward
    });
